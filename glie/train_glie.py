...
def training_step(model, corpus, art, q_idx, docs, weights, cfg, device, support_dirs):
    ...
    if model.decoder is not None:
        gen = model.generate(refined.detach() if getattr(cfg, "detach_code", False) else refined,
                             raw_norm, ab["count_feature"], ab["slot_owner"],
                             ab["slot_fraction"], ab["anchor_mask"], ab["slot_rank"])
...
def train_one(corpus, art, k, cfg, device, out_dir):
    ...
    model = GLIE(corpus.dim, cfg.refiner, cfg.decoder, cfg.heads, cfg.dec_hidden,
                 ...
                 use_count=not cfg.no_count_channel,
                 decoder_rank=getattr(cfg, "decoder_rank", 0),
                 decoder_layers=getattr(cfg, "dec_layers", 1),
                 decoder_child_dims=getattr(cfg, "child_dims", 1)).to(device)
    ...
    result = {
        ...
        # Recorded so a results file says what shape produced it. Without this a width or depth
        # sweep leaves several CSVs that differ only by a flag nobody wrote down.
        "dec_hidden": int(cfg.dec_hidden), "dec_layers": int(getattr(cfg, "dec_layers", 1)),
        "decoder_rank": int(getattr(cfg, "decoder_rank", 0)),
        "child_dims": int(getattr(cfg, "child_dims", 1)),
        "detach_code": bool(getattr(cfg, "detach_code", False)),
...
def evaluate_transfer(ckpt_path, corpus, art, k, cfg, device):
    ...
    model = GLIE(corpus.dim, cfg.refiner, cfg.decoder, cfg.heads, cfg.dec_hidden,
                 ...
                 decoder_layers=getattr(cfg, "dec_layers", 1),
                 decoder_child_dims=getattr(cfg, "child_dims", 1)).to(device)
    if getattr(cfg, "eval_all_queries", False):
