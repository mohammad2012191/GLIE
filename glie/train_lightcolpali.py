...
def build_args():
    ...
    p.add_argument("--eval_all_queries", action="store_true",
                   help="STANDARD-PROTOCOL EVAL: score every query of each --datasets corpus, as "
                        "the official benchmark does, matching run_main.py's flag of the same "
                        "name. Valid here because the adapter is fitted on --train_dataset "
                        "(vidore/colpali_train_set) and never sees a target query, so there is "
                        "nothing to hold out. Training still uses --train_dataset only.")
...
def main():
    ...
    if a.eval_all_queries:
        splits = {d: (splits[d][0], np.arange(corpora[d].n_queries)) for d in corpora}
        print(f"  [eval] standard protocol: scoring ALL queries "
              f"({sum(c.n_queries for c in corpora.values())} total across "
